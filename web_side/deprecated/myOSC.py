...
from socketserver import UDPServer, TCPServer, BaseRequestHandler, DatagramRequestHandler, ForkingMixIn, ThreadingMixIn
...
class OSCMessage(object):
	...
	def append(self, argument, typehint=None):
		...
		self.typetags += tag
		self.message += binary.decode('ascii')
	...
	def getBinary(self):
		...
		binary += bytes(self.message, 'ascii')
...
def OSCString(next):
	...
	return struct.pack(">%ds" % (OSCstringLength), bytes(next, 'ascii'))
...
def OSCBlob(next):
	...
	if type(next) in str:
		...
		binary = struct.pack(">i%ds" % (OSCblobLength), OSCblobLength, bytes(next, 'ascii'))
	else:
		binary = ""
...
class MyTCPHandler(BaseRequestHandler):
	...
	def handle(self):
		global TCPlibrary
		TCPlibrary[self.client_address[0]] = []
		while True:
			try:
				if len(TCPlibrary[self.client_address[0]]) > 0:
					# send back the archived data
					self.request.sendall(str(TCPlibrary[self.client_address[0]].pop(0)))
					#self.request.sendall((TCPlibrary[self.client_address[0]].pop(0)).getBinary())
			except:
				#print "MyTCPHandler exception!"
				break
...
class OSCClient(object):
	...
	"""Simple OSC Client. Handles the sending of OSC-Packets (OSCMessage or OSCBundle) via a UDP-socket
	   - 2012-02-21 - added partial support for TCP-sockets - BJP
	"""
	# set outgoing socket buffer size
	sndbuf_size = 4096 * 8

	def __init__(self, server=None, addr=None, TCPaddr=None):
		global TCPlibrary
		...
		self.socket = None
		self.TCPserver = None
		if TCPaddr is not None:
			self.TCPserver = TCPServer(TCPaddr, MyTCPHandler)
	...
	def close(self):
		global TCPlibrary
		"""Disconnect & close the Client's sockets
		"""
		if self.socket != None:
			self.socket.close()
			self.socket = None
		if self.TCPserver != None:
			TCPlibrary = {}
			self.shutdownThread = threading.Thread( target = self.TCPserver.shutdown )
	...
	def sendto(self, msg, address, timeout=None):
		...
		# add this to all TCP listeners
		if address[0] in TCPlibrary:
			TCPlibrary[address[0]].append(msg)
	...
	def send(self, msg, timeout=None):
		...
		# send to all TCP listeners:
		for addr in TCPlibrary:
			TCPlibrary[addr].append(msg)
