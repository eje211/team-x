...
public class Osc : MonoBehaviour {
	...
	public static OscMessage StringToOscMessage(string message) {
		...
		if (sE.MoveNext()) oM.Address = (string)sE.Current;
		...
		while (sE.MoveNext()) {
			...
			if (s.StartsWith("\"")) {
				...
				bool looped = false;
				if (s.Length > 1) quoted.Append(s.Substring(1));
				else looped = true;
				while (sE.MoveNext()) {
					string a = (string)sE.Current;
					// Console.WriteLine("    q:<" + a + ">");
					if (looped) quoted.Append(" ");
					if (a.EndsWith("\"")) {
	...
	private static string Dump(byte[] packet, int start, int length) {
		StringBuilder sb = new StringBuilder();
		int index = start;
		while (index < length) sb.Append(packet[index++]+"|");
		return sb.ToString();
	}
	...
	private static int PadSize(int rawSize) {
		int pad = rawSize % 4;
		if (pad == 0) return rawSize;
		else return rawSize + (4 - pad);
	}
}
//}